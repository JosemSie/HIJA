...
public class Equities {
	...
	private ArrayList<Carta> cartaInsertb;//grupos de 5 cartas que juntaremos con las distintas manos
	private Carta[] mesaInsert;
	...
	public void generaCartaInsert() {//devolvera una lista de todas permitaciones de 5 cartas permitidas	
		int k = this.cartasMesa.size();
		this.cartaInsertb = new ArrayList<Carta>();
		this.mesaInsert = new Carta[NCARTASMESA];//el array de cartas que vamos a insertar n veces,
		for(int i=0;i<this.cartasMesa.size();i++)
			this.mesaInsert[i] = this.cartasMesa.get(i);
		
		generaCartaInserRecursiv(k, 0, 0);
	}
	
	//genera la lista 5 en cinco cartas de todas la baraja que aun no ha sido repartida
	//k = numero de cartas ya en la mesa, p= el palo de la ultima carta insertada, el numero de la ultima carta insertada
	private void generaCartaInserRecursiv(int k, int p, int v){//solo variaran las que no son de la mesa inicial, por lo que pasando la proxima posicion a cambiar de un array nos ahoramos mucha memoria 
		if(k==NCARTASMESA) {//cuando hemos generado una mesa la añadimos
			this.cartaInsertb.addAll(new  ArrayList<>(Arrays.asList(this.mesaInsert)));
		}
				...
				for(int i=v+1; i<13;i++) {//23456789tjqka
					for(int j=p+1; j<4;j++) {//las cartas se intentaran insertar desde 2h, 2d... hasta ...Ac, As
						if(!this.cartasMarc[i][j]) {
							this.mesaInsert[k] = new Carta(i+2,conver.intAPalo(j));
